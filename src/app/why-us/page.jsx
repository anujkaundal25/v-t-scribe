...
export default function WhyUsPage() {
  return (
    <div className="bg-slate-50 text-slate-800 font-sans min-h-screen pt-24 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {keyBenefits.map((item, idx) => (
            <motion.div 
              key={idx}
              whileHover={{ y: -5 }}
              className="bg-white p-7 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all space-y-4 flex flex-col justify-between"
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">
                    {item.highlight}
                  </span>
                </div>
      ...
      <section className="py-12 bg-sky-900 text-white px-6">
      ...
      <section className="py-20 bg-slate-100/80 border-t border-slate-200 px-6">
        <div className="max-w-7xl mx-auto">
            ...
            {workflowSteps.map((step, idx) => (
              <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4 flex flex-col justify-between">
                <div className="space-y-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900">{step.role}</h3>
          ...
          {/* Call to Action Bar */}
          <div className="mt-16 bg-gradient-to-r from-[#002742] to-[#014f92] text-white p-8 rounded-3xl shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
            ...
            <a 
              href="#contact" 
              className="bg-sky-500 hover:bg-sky-400 text-white font-bold px-6 py-3.5 rounded-xl shadow-lg transition-all shrink-0 text-sm flex items-center gap-2"
            >
